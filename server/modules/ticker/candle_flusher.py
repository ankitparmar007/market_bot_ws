...
class CandleFlusher:
    ...
    async def flush_candles_to_db():
        ...
        for instrument_key in instrument_keys:
            ...
            ticks = await TicksCollections.ticks.find(
                {
                    ...
                    "timestamp": {
                        "$gte": ISDateTime.market_start_utc(),
                        "$lte": ISDateTime.market_end_utc(),
                    },
                }
            )
            ...
            volume_history = VolumeTicker.process_volume_ticks(ticks=ticks)
        ...
        log.info(
            f"CandleFlusher: Flushed candles to DB. Volume modified: {res2.modified_count}, Volume upserted: {res2.upserted_count}"
        )
        # log.info(
