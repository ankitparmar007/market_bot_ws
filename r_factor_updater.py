...
from utils import IndianDateTime
...
async def update_r_factor_loop():

    START_TIME = IndianDateTime.now().replace(
        hour=9, minute=16, second=3, microsecond=0
    )
    END_TIME = IndianDateTime.now().replace(hour=15, minute=30, second=0, microsecond=0)
        ...
        while True:
            ...
            try:
                response = await client.get_json(
                    ...
                    headers={"Content-Type": "application/json", "token": "valid123"},
                )
